...
@section('title', $policy->title . ' - Chính Sách - ' . config('app.name'))
@section('description', strip_tags(substr($policy->content, 0, 160)))
        ...
        <div class="row">
            ...
            <div class="col-lg-9 col-md-8">
                <div class="policy-content">
                    <h1 class="policy-content-title">{{ $policy->title }}</h1>
                    <div class="policy-body" id="policyBody">
                        {!! html_entity_decode($policy->content ?? '', ENT_QUOTES, 'UTF-8') !!}
                    </div>
                </div>
            </div>
        </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const content = document.getElementById('policyBody');
