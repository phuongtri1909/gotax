...
return new class extends Migration
{
    ...
    {
        Schema::create('go_invoice_purchases', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user_id')->nullable()->constrained()->nullOnDelete();
            $table->foreignId('bank_id')->nullable()->constrained()->nullOnDelete();
            $table->foreignId('package_id')->constrained('go_invoice_packages')->cascadeOnDelete();
            $table->string('transaction_code')->unique();
            $table->decimal('amount', 10, 2);
            $table->integer('mst_limit');
            $table->decimal('license_fee', 10, 2)->nullable();
            $table->timestamp('expires_tool')->nullable();
    ...
    {
        Schema::dropIfExists('go_invoice_purchases');
    }
};
