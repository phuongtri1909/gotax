...
class GoSoftPackageController extends Controller
{
    /**
    ...
    {
        $packages = GoSoftPackage::ordered()->paginate(10);
        return view('admin.pages.go-soft-packages.index', compact('packages'));
    }
    ...
    {
        return view('admin.pages.go-soft-packages.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        GoSoftPackage::create($data);

        return redirect()->route('admin.go-soft-packages.index')
            ->with('success', 'Gói GoSoft đã được tạo thành công.');
    }
     ...
     */
    public function edit(GoSoftPackage $goSoftPackage)
    {
        return view('admin.pages.go-soft-packages.edit', compact('goSoftPackage'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, GoSoftPackage $goSoftPackage)
    {
        $request->validate([
            'name' => 'required|string|max:255',
            'price' => 'required|numeric|min:0',
        ...
        $data = $request->except(['features_text']);
        if ($request->name !== $goSoftPackage->name) {
            $data['slug'] = Str::slug($request->name);
        }
        ...
        $goSoftPackage->update($data);

        return redirect()->route('admin.go-soft-packages.index')
            ->with('success', 'Gói GoSoft đã được cập nhật thành công.');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(GoSoftPackage $goSoftPackage)
    {
        $goSoftPackage->delete();

        return redirect()->route('admin.go-soft-packages.index')
            ->with('success', 'Gói GoSoft đã được xóa thành công.');
    }
}
